...
export default async function CollectionDetailPage({
  ...
  const { products: all } = await getCatalogProducts();
  const products = filterByCollection(all, collection.slug);
    ...
    <main className="relative w-full flex flex-col items-center">
      ...
      <section className="relative w-full h-[60vh] min-h-[420px] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl px-6 text-center flex flex-col items-center">
          ...
          <h1 className="font-heading text-5xl md:text-6xl text-[#F5F2EF] mb-6">
            {collection.name}
          </h1>
      ...
      <ShopExplorer
        products={products}
        title={`${products.length} Pieces in this Collection`}
      />
    </main>
  );
}
