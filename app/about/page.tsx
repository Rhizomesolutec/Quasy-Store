    ...
    <main className="relative w-full flex flex-col items-center">
      ...
      <section className="w-full px-4 md:px-12 lg:px-24 py-20 max-w-6xl">
        <div className="grid md:grid-cols-2 gap-12 md:gap-20 items-center">
          ...
          <RevealOnScroll delay={0.1}>
            ...
            <h2 className="font-heading text-3xl md:text-4xl text-[#F5F2EF] mb-6 leading-tight">
            ...
            <p className="font-sans text-sm md:text-base text-[#F5F2EF]/60 leading-relaxed">
      ...
      <section className="w-full px-4 md:px-12 lg:px-24 py-20 max-w-4xl text-center border-t border-white/[0.06]">
        <RevealOnScroll>
          ...
          <h2 className="font-heading text-3xl md:text-5xl text-[#F5F2EF] mb-6 leading-tight">
            Jewelry that carries weight — literally and otherwise.
          </h2>
          <Divider className="mx-auto mb-6" />
          <p className="font-sans text-base text-[#F5F2EF]/60 leading-relaxed max-w-2xl mx-auto">
      ...
      <section className="w-full px-4 md:px-12 lg:px-24 py-20 max-w-6xl border-t border-white/[0.06]">
        <RevealOnScroll>
          <h2 className="font-heading text-2xl text-[#F5F2EF] text-center mb-12">What We Value</h2>
        </RevealOnScroll>
        <div className="grid md:grid-cols-3 gap-8">
          {VALUES.map((value, idx) => (
            <RevealOnScroll key={value.title} delay={idx * 0.1}>
              <div className="border border-white/[0.08] p-8 h-full">
                <span className="font-heading text-4xl text-[#E50914]/30 block mb-4">{String(idx + 1).padStart(2, "0")}</span>
                <h3 className="font-heading text-xl text-[#F5F2EF] mb-3">{value.title}</h3>
                <p className="font-sans text-sm text-[#F5F2EF]/60 leading-relaxed">{value.body}</p>
              </div>
            </RevealOnScroll>
          ))}
        </div>
      ...
      <section className="w-full px-4 md:px-12 lg:px-24 py-20 max-w-4xl border-t border-white/[0.06]">
        <RevealOnScroll>
          <h2 className="font-heading text-2xl text-[#F5F2EF] text-center mb-16">A Century of Silence, Then One Reissue</h2>
        </RevealOnScroll>
        <div className="relative pl-8 border-l border-[#E50914]/30 space-y-12">
          {TIMELINE.map((item, idx) => (
            <RevealOnScroll key={item.year} delay={idx * 0.08} className="relative">
              <span className="absolute -left-[calc(2rem+5px)] top-1 w-2.5 h-2.5 rounded-full bg-[#E50914]" />
              <span className="font-heading text-sm text-[#E50914] tracking-widest">{item.year}</span>
              <h3 className="font-heading text-xl text-[#F5F2EF] mt-1 mb-2">{item.title}</h3>
              <p className="font-sans text-sm text-[#F5F2EF]/60 leading-relaxed max-w-lg">{item.body}</p>
            </RevealOnScroll>
          ))}
        </div>
      </section>
      ...
      <section className="w-full px-4 md:px-12 lg:px-24 py-20 max-w-6xl border-t border-white/[0.06]">
        <RevealOnScroll>
          <h2 className="font-heading text-2xl text-[#F5F2EF] text-center mb-12">Why Choose Qusay Store</h2>
        </RevealOnScroll>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {WHY_US.map((item, idx) => (
            <RevealOnScroll key={item.title} delay={idx * 0.08}>
              <div className="text-center px-2">
                <h3 className="font-heading text-base text-[#F5F2EF] mb-2">{item.title}</h3>
                <p className="font-sans text-xs text-[#F5F2EF]/50 leading-relaxed">{item.body}</p>
              </div>
            </RevealOnScroll>
          ))}
        </div>
      ...
      <section className="w-full px-4 md:px-12 lg:px-24 py-24 max-w-3xl text-center border-t border-white/[0.06]">
        <RevealOnScroll>
          <h2 className="font-heading text-3xl md:text-4xl text-[#F5F2EF] mb-6">Explore the Full Catalog</h2>
          <p className="font-sans text-sm text-[#F5F2EF]/60 mb-8 leading-relaxed">
